<?php
// $Id: mod_folder.php,v 1.4 2005/03/14 20:05:43 jsaucier Exp $
//  ------------------------------------------------------------------------ //
//                           Document Manager                                //
...
// Set the template
$xoopsOption['template_main'] = 'docmanager_modfolder.html';    
...
// Test if the folder exist, if we have the right to access it, if we can mod the folder and if the folder is not hidden
if (if_folder_exist($current_dir) && have_access_right_to_folder($current_dir) && have_mod_right_to_folder($current_dir) && (is_not_hidden($current_dir) || $is_mod_admin)) {


    // Check if we dont try to mod the parent folder
    if ( $current_dir != 1) {
        ...
        //Display the error
        $xoopsTpl->assign('docmanager_error', _MD_MODFOLDER_PARENT);
        $error = 1;
    }
}
else {
...
// Set langage options
$xoopsTpl->assign('docmanager_title', _MD_DOCMANAGER_MODFOLDER);
$xoopsTpl->assign('docmanager_folder_name', $myrow['folders_name']);
$xoopsTpl->assign('docmanager_modfolder_name', _MD_MODFOLDER_NAME);
$xoopsTpl->assign('docmanager_modfolder_submit', _MD_MODFOLDER_SUBMIT);
$xoopsTpl->assign('docmanager_modfolder_cancel', _MD_MODFOLDER_CANCEL);
